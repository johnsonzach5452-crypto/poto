...
if st.button("Scan now", type="primary"):
    ...
    # ============ PITCHER PROPS ============
        ...
        # 2) Odds API events (cheap) to get event ids for prop calls
        try:
            oa_events = list_events(league, api_key)
        except Exception as e:
            st.error(f"Odds API events failed: {type(e).__name__}: {e}"); st.stop()
        ...
        if not pairs:
            st.warning("No games matched between feeds; can't scan props.")
            st.stop()
